...
public final class LatLongUtils {
	 ...
	 *         tile size.
	 */
	public static byte zoomForBounds(Dimension dimension, BoundingBox boundingBox, MapProjection projection) {
		double pixelXMax = projection.longitudeToPixelX(boundingBox.maxLongitude);
		double pixelXMin = projection.longitudeToPixelX(boundingBox.minLongitude);
		double zoomX = -Math.log(Math.abs(pixelXMax - pixelXMin) / dimension.width) / Math.log(2);
		double pixelYMax = projection.latitudeToPixelY(boundingBox.maxLatitude);
		double pixelYMin = projection.latitudeToPixelY(boundingBox.minLatitude);
